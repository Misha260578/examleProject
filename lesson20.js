...
btn.addEventListener("click",  ()=>{
    if (title.value.trim() && aname.value.trim()) {
        dataBase.users.push({name: aname.value, selery: title.value})
        title.value = ''
        aname.value = ''
        rander()
    }
 ...
 container.addEventListener("click", (e)=>{
    if(e.ltarget.closest(".user-delete")){
        const ikon = e.target.closest(".user-delete"),
        svg = +ikon.dataset.delete;
        
   
        deleteUser(svg, dataBase.users, rander)
    }
 })
 
function deleteUser(index, array, callback){
